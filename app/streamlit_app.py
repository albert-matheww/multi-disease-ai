...
from src.prediction import DiseasePredictor, get_predictor
...
def render_sidebar() -> tuple[str, str, bool]:
    with st.sidebar:
        ...
        st.divider()
        st.caption("Model: TabPFN v2 (foundation model for tabular data)")
...
def page_predict(disease_key: str) -> None:
    ...
    with gauge_col:
        st.plotly_chart(
            render_probability_gauge(result.probability, result.risk_level), use_container_width=True
        )
    with chart_col:
        st.plotly_chart(render_contributor_chart(result.top_contributors), use_container_width=True)
...
def page_model_performance(disease_key: str) -> None:
    ...
    metrics = json.loads(disease.metrics_path.read_text())
    cols = st.columns(5)
...
(a tabular foundation model with SHAP explainability) rather than a
production diagnostic system. See the main `README.md` for the full
