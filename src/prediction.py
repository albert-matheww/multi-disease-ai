...
structured `PredictionResult` with probability, risk level, confidence, and
a SHAP-based explanation - everything `report.py` needs to render a report
and everything the dashboard needs to render a result card.
"""
...
from src.config import RISK_THRESHOLDS, DiseaseConfig, get_disease
...
class DiseasePredictor:
    ...
    def explainer(self) -> DiseaseExplainer:
        if self._explainer is None:
            ...
            self._explainer = DiseaseExplainer(self.disease.key, self.model, self.feature_order, X_train)
    ...
    def predict(self, raw_record: dict, explain: bool = True, top_n: int = 5) -> PredictionResult:
        ...
        if explain:
            explanation = self.explainer.explain_instance(x_encoded, top_n=top_n)
    ...
    def predict_batch(self, records_df: pd.DataFrame) -> pd.DataFrame:
        ...
        `predicted_label`, and `risk_level` columns appended. Explanations
        are omitted for batch scoring (too slow to compute SHAP per row for
        large uploads); use `.predict()` for a single-record explanation.
        """
