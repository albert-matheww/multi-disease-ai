...
\usepackage{amsmath,amssymb,graphicx,booktabs,tabularx,array,xcolor,url,cite}

\newcommand{\pending}[1]{\textbf{\textcolor{red}{[PENDING#1]}}}

\title{MultiDiseaseAI: Evaluating TabPFN-Based Design Choices for Multi-Disease Clinical Risk Prediction}

\author{\IEEEauthorblockN{1\textsuperscript{st} Albert Mathew Suni}\IEEEauthorblockA{\textit{Reg. No. 23BCE1765} \\ Vellore Institute of Technology \\ Chennai, India}\and
\IEEEauthorblockN{2\textsuperscript{nd} Nandhitha S H}\IEEEauthorblockA{\textit{Reg. No. 23BCE1552} \\ Vellore Institute of Technology \\ Chennai, India}\and
\IEEEauthorblockN{3\textsuperscript{rd} Dr. Vijayraj J}\IEEEauthorblockA{\textit{Project Guide, Employee ID 54799} \\ Vellore Institute of Technology \\ Chennai, India}}
...
\item A description of the system as implemented, including the exact form of its uncertainty band: a global-width residual band whose target is the binary label, which read as a set of labels is a conformal label set (Section III).

\item Head-to-head measurements of six design choices, including TabPFN itself against four reference models, with paired tests that account for overlapping resamples (Section V).

\item A reproducible, provenance-stamped harness that reruns identically once new evidence (a licence token, a new dataset) is available.
...
\textbf{Tabular foundation models and clinical evidence.} Prior-data fitted networks train a set-input transformer on tasks sampled from a prior so that one forward pass approximates a posterior predictive distribution \cite{muller2022}. The first TabPFN applied this to small numeric datasets, with at most 1,000 training rows, 100 numeric features and 10 classes \cite{hollmann2023}. Its 2025 successor extends the range to 10,000 samples and 500 features, handles categorical features and missing values, and reports its advantage against baselines tuned for hours \cite{hollmann2025}. Earlier evidence that tree ensembles stay ahead of neural networks at around ten thousand samples \cite{grinsztajn2022} predates this generation. Clinical evidence is mixed, as the Introduction noted; both studies were read at abstract level only, and the first is not peer reviewed. This project does not fine-tune, and it keeps reference models for that reason.

\textbf{Explaining TabPFN.} SHAP defines additive feature attributions in a unified framework \cite{lundberg2017}. TabPFN-specific methods exploit in-context learning to compute Shapley values and leave-one-covariate-out importances without retraining \cite{rundel2024}, and one workshop paper proposes a model that emits its own Shapley explanation in a single pass, reporting model-agnostic KernelSHAP as far slower in its setting \cite{realtime2026} --- a finding Section V-F tests directly. Shapley importances also carry conceptual caveats \cite{kumar2020}. We use the model-agnostic permutation explainer with a k-means background to bound cost, and do not compare it with the TabPFN-specific methods.

\textbf{Uncertainty, decision thresholds and novelty.} The jackknife interval adds and subtracts a quantile of leave-one-out residuals and can have very poor coverage in some settings; jackknife+ and CV+ repair this with a worst-case coverage guarantee of about 1$-$2$\alpha$ but need every fold model at the test point \cite{barber2021}. Cross-conformal prediction was studied largely empirically \cite{vovk2015}; marginal coverage is what conformal methods deliver, and how informative the sets are depends on the score function \cite{angelopoulos2021}. Mahalanobis distance on class-conditional Gaussians is used for out-of-distribution detection in deep networks \cite{lee2018}; we fit one shrunk Gaussian \cite{ledoit2004} on encoded inputs instead. The Youden index weights sensitivity and specificity equally \cite{shan2015}, and modern neural networks are often poorly calibrated \cite{guo2017}, which we did not assess. Our band is a K-fold analogue of the jackknife interval, tested here against CV+ and a label-set formulation.

\textbf{Evaluation practice and the data.} Leakage errors have been reported in 17 research fields, touching 329 papers, with some conclusions badly overoptimistic; one catalogued form is preprocessing fitted on training and test data together \cite{kapoor2023}. Small samples give wide error bars that fold-to-fold spread understates \cite{varoquaux2018}, and variance estimates that ignore the choice of training set can grossly underestimate the variance of resampled comparisons, which the corrected resampled t-test addresses \cite{nadeau2003}. We adopt that test, Wilson intervals \cite{brown2001} and percentile bootstrap intervals for the fixed split; improved bootstrap intervals exist \cite{diciccio1996}. Applied work on these datasets includes a 12-classifier comparison reporting 0.983 accuracy for gradient-boosted trees on kidney disease \cite{islam2023} and a heart-disease system with feature engineering \cite{bouqentar2024}; a study that recreated published liver-disease classifiers found a higher false-negative rate for female patients across all of them \cite{straw2022}.

\section{System Design}

Fig. 1 shows the pipeline. Everything the model needs at serving time is in one bundle per disease.

\begin{figure}[!t]\centering\includegraphics[width=\columnwidth]{figures/fig1_pipeline.pdf}\caption{Pipeline as implemented in \texttt{src/train.py} and \texttt{src/prediction.py}. The out-of-fold pass runs once at training time; serving needs one model forward pass.}\label{fig:pipeline}\end{figure}

\textbf{Data and preprocessing.} A configuration file registers each disease's columns, target and form fields, so adding a disease means adding an entry. The order is deterministic cleaning, one stratified 80/20 split (seed 42), median or most-frequent imputation, 1.5$\times$IQR winsorization, ordinal encoding and feature engineering. Every fitted statistic comes from the training split, a unit test checks the winsorization bounds against the imputed training split, and a saved artifact replays the identical transform on a live record. The engineered features are the rate-pressure product, heart-rate reserve and cholesterol/age (heart); glucose$\times$BMI and insulin/glucose (diabetes); a comorbidity count (kidney); and AST/ALT and direct/total bilirubin (liver).

\textbf{Model.} Each disease gets a \texttt{TabPFNClassifier} with \texttt{n\_estimators=8} and \texttt{random\_state=42} on CPU with categorical column indices, package version 8.2.0, and no hyperparameter search. TabPFN is the only classifier in the shipped system.

\textbf{Precomputed add-ons.} A stratified five-fold out-of-fold pass over the training split (folds capped at the smaller class) yields three artifacts.

\emph{Band.} With out-of-fold probabilities $p_i$ and labels $y_i \in \{0,1\}$, the half-width $h$ is the $\lceil (n+1)(1-\alpha) \rceil$-th smallest residual $|y_i - p_i|$, with $\alpha = 0.10$. A patient with probability $p$ receives the band $[p-h,\, p+h]$ clipped to $[0,1]$. Two properties matter for interpretation. First, $h$ is one constant per disease, so the band is not adaptive to the patient. Second, for a binary label $|y-p| = 1-p_y$, where $p_y$ is the probability assigned to the true label, so the band contains label $y$ exactly when $1-p_y \le h$. Read as the set of labels it contains, the band is therefore the conformal label set with score $1-p_y$. Its target is coverage of the label, not of the unknown true probability, and it is approximate because fold-model residuals are applied to the full-data model. It resembles the plain jackknife interval \cite{barber2021} computed from K folds; it is not CV+.

\emph{Threshold.} The decision threshold is the out-of-fold probability that maximises Youden's $J$ = sensitivity + specificity $-$ 1 \cite{shan2015}, clamped to [0.01, 0.99].

\emph{Novelty flag.} A Ledoit--Wolf-shrunk Gaussian is fit to the encoded training matrix, and a live record is flagged when its Mahalanobis distance exceeds the 97.5\% training-distance quantile.

\textbf{Recorded rationale.} The repository states its reasons for these choices in code comments and documentation. TabPFN was specified as the only classifier because, on small mixed-type tables, it was reported to match tuned boosted trees without a search that would otherwise be needed per disease. The Youden threshold replaces a blind 0.5 cutoff, computed out-of-fold so it never touches the test split. The band replaces ``confidence as distance from 0.5'' with an interval. The novelty flag exists because every source dataset is small and demographically narrow. The interactive SHAP background was cut from 25 to 10 rows for latency, unbenchmarked when this project started; Section V-F reports the benchmark.

\textbf{Serving and explanations.} A prediction is one forward pass, a band lookup, a threshold comparison and a novelty distance. The label is $p \ge$ threshold. SHAP values come from a permutation explainer on \texttt{predict\_proba} with a k-means background of 25 rows for global plots and 10 for interactive use \cite{lundberg2017}. The application adds batch scoring, PDF reports, a SQLite prediction history and Docker and Hugging Face Space files; it is meant to run locally, and we made no deployment. Eighty-one tests pass, but the prediction wiring is tested with a logistic-regression stand-in rather than TabPFN.

\section{Experimental Setup}

\textbf{Data.} Table I lists the datasets, all public. Heart disease uses the Cleveland subset of the UCI data \cite{uci_heart,detrano1989}. Diabetes is the Pima Indians data, whose documentation describes 768 female patients aged at least 21 of Pima heritage \cite{pima_docs}. Kidney disease is the UCI CKD data \cite{uci_ckd} and liver disease is the Indian Liver Patient Dataset \cite{uci_ilpd}. Cleaning matters: 5, 35, 227, 374 and 11 Pima zeros in glucose, blood pressure, skin thickness, insulin and BMI are treated as missing; kidney missingness reaches 152 of 400 rows (\texttt{rbc}); heart has 4 missing \texttt{ca} and 2 missing \texttt{thal}; and ILPD has 13 duplicate rows, which are dropped, and 4 missing A/G ratios.

\begin{table}[!t]
\caption{Datasets after cleaning. Test positives are counted in the shipped 20\% split.}
\label{tab:data}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
\textbf{Disease} & \textbf{Rows} & \textbf{Features} & \textbf{Train / test} & \textbf{Positive rate} & \textbf{Test positives} \\
\midrule
...
\begin{table}[!t]
\caption{Cross-validated ROC-AUC (5 folds $\times$ 3 repeats) and wall-clock seconds per outer fold, including the reference models' inner tuning; TabPFN gets no search. CPU.}
\label{tab:models}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & \textbf{Heart} & \textbf{Diabetes} & \textbf{Kidney} & \textbf{Liver} & \textbf{Seconds/fold} \\
\midrule
TabPFN & \textbf{0.918} & \textbf{0.843} & 1.000 & 0.741 & 1.0--1.8 \\
Logistic regression & 0.909 & 0.839 & 1.000 & \textbf{0.744} & 0.05 \\
...
Numerically ahead is not the same as significantly ahead. Table III gives the paired Nadeau--Bengio test, TabPFN minus each reference model, over the same 15 folds. TabPFN was significantly better than the SVM on three of the four tasks (kidney is untestable, both scoring 1.000 with zero variance) and significantly better than gradient boosting on one (diabetes, +0.0125, p = 0.047), but never significantly different from logistic regression (p = 0.145--0.734) or random forest (p = 0.071--0.653) on any task. This matches the clinical-benchmark preprint's finding: generally competitive, decisively ahead of the weaker baseline, not of the strongest one \cite{clinbench2026}.

\begin{table}[!t]
\caption{TabPFN minus reference model, mean AUC difference and Nadeau--Bengio p (15 folds; kidney vs SVM has zero variance, no test defined).}
\label{tab:tabpfn-vs-ref}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Disease} & \textbf{vs HGB} & \textbf{vs LogReg} & \textbf{vs RF} & \textbf{vs SVM} \\
\midrule
Heart & +0.017 (p=.104) & +0.009 (p=.145) & +0.015 (p=.071) & \textbf{+0.022 (p=.029)} \\
Diabetes & \textbf{+0.013 (p=.047)} & +0.004 (p=.396) & +0.004 (p=.378) & \textbf{+0.017 (p=.007)} \\
Kidney & +0.000 (p=.414) & +0.000 (p=.653) & +0.000 (p=.653) & +0.000 (n/a) \\
Liver & +0.019 (p=.068) & $-$0.003 (p=.734) & +0.007 (p=.498) & \textbf{+0.081 (p$<$.001)} \\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Preprocessing and leakage}
...
\begin{table*}[!t]
\caption{Change in ROC-AUC relative to the project's preprocessing (variant minus project); smallest Nadeau--Bengio p over pairs with a defined test.}
\label{tab:preproc}
\centering\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}Xcccccc}
\toprule
\textbf{Variant} & \textbf{Ref. pairs} & \textbf{Ref. $\Delta$AUC range} & \textbf{Ref. min p} & \textbf{TabPFN pairs} & \textbf{TabPFN $\Delta$AUC range} & \textbf{TabPFN min p} \\
\midrule
...
\begin{table}[!t]
\caption{Balanced accuracy by decision rule, logistic regression / gradient boosting / TabPFN, mean over outer folds (15 for the reference models, 5 for TabPFN).}
\label{tab:threshold}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Rule} & \textbf{Heart} & \textbf{Diabetes} & \textbf{Kidney} & \textbf{Liver} \\
\midrule
...
We evaluated the band and two alternatives at $\alpha$ = 0.10 by reading each as a set of labels: the project band, CV+ \cite{barber2021} and the conformal label set with score $1-p_y$ (Table VI), for the two reference models and TabPFN. The band and the label-set method gave identical coverage, set size and singleton, both-label and empty rates in all twelve model--disease pairs, as Section III implies. Coverage was 0.900 to 0.920 for the reference models and 0.901 to 0.930 for TabPFN, against a nominal 0.90. CV+ covered 0.898 to 0.929 (reference models) and 0.904 to 0.912 (TabPFN), within 0.018 of the band across all three models. On the tuned reference models of the main comparison, the band's outer-fold label coverage was 0.897 to 0.920 and every Wilson 95\% interval contained 0.90.

\begin{table}[!t]
\caption{Label coverage (nominal 0.90) and share of singleton label sets, logistic regression / gradient boosting / TabPFN. The band and the label-set method are identical.}
\label{tab:band}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Quantity} & \textbf{Heart} & \textbf{Diabetes} & \textbf{Kidney} & \textbf{Liver} \\
\midrule
Coverage, band & .906/.904/.901 & .900/.905/.904 & .918/.920/\textbf{.930} & .904/.917/.905 \\
Coverage, CV+ & .905/.913/.904 & .898/.911/.908 & .907/.922/.912 & .910/.929/.912 \\
Singleton, band & .815/.766/\textbf{.862} & .670/.648/.697 & .918/.920/\textbf{.930} & .580/.526/.567 \\
\bottomrule
\end{tabular}}
\end{table}

Validity is not informativeness \cite{angelopoulos2021}. The singleton-set share, where the band excludes one label, ranged from 0.53 (liver, gradient boosting) to 0.93 (kidney, TabPFN). On liver about half of predictions carry both labels for every model: the band says ``either'', a weak classifier rather than a flaw in the band. On kidney disease TabPFN's own residuals nearly vanish --- mean band width 0.001, against 0.012--0.084 for the reference models --- because the fitted probabilities cluster at 0 or 1 with almost no out-of-fold error on this near-separable task; all three kidney worked examples in Section V-F land on an exact [0,0] or [1,1] band. That is the band behaving as designed, not a defect, but it leaves no calibrated margin for the rare kidney patient the model gets wrong; on the harder tasks the band stayed wide (0.80--0.88 for TabPFN). A lookup costs 0.53 $\mu$s regardless of model. CV+ needs all K fold models at the query point \cite{barber2021}, so serving it would take five forward passes instead of one, by definition; we did not time it. Not tested: coverage within subgroups, and calibration of the underlying probabilities.

\subsection{Novelty flag}
...
\begin{table*}[!t]
\caption{Novelty detectors. Flag rate on held-out normal rows in 5-fold CV (nominal 0.025), age-shift AUROC, and query time per row.}
\label{tab:novelty}
\centering\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}Xccc}
\toprule
\textbf{Detector} & \textbf{CV flag rate} & \textbf{AUROC: kidney / diabetes / heart / liver} & \textbf{$\mu$s per row} \\
\midrule
...
\begin{figure}[!t]\centering\includegraphics[width=\columnwidth]{figures/fig2_novelty.pdf}\caption{Held-out flag rate against age-shift AUROC for six detectors on four datasets; the project's detector (*) sits near the nominal rate.}\label{fig:novelty}\end{figure}

The project's detector held its nominal rate on all four datasets (0.030 to 0.036) and was among the cheapest, at 9 to 14 $\mu$s per row (the empirical-covariance variant is equally cheap) against 92 to 1,552 $\mu$s for the other four. It was not the most sensitive. The one-class SVM scored a higher AUROC on all four datasets but flagged 11\% to 24\% of normal held-out rows, so its threshold is not usable without recalibration. Shrinkage helped: the empirical covariance flagged 8.7\% of held-out kidney rows against 3.0\%, and its liver AUROC was 0.798 against 0.909. On heart disease the project flag rose from 6.1\% of younger held-out patients to 57.4\% of older ones. On kidney disease no detector separated the age groups well (AUROC 0.64 to 0.69), so the flag's response there is unproven.

\subsection{Explanations and sample predictions}

We timed a single-row \texttt{predict\_proba} call in a loop over the first 20 test rows of each disease, then the permutation explainer at background sizes 10 and 25 on the same rows, then KernelSHAP at background 10 for comparison (Table VIII). A single unbatched prediction cost 0.70 to 1.17 seconds. A separate, direct comparison on the same fitted model --- the same 20 rows scored one at a time versus batched --- found the single-row path 18 to 20 times slower per row (581--1,144 ms singly against 32--59 ms per row batched). The interactive, one-patient-at-a-time dashboard path therefore does not get TabPFN's batching efficiency; a form submission pays close to the full per-call overhead every time.

\begin{table*}[!t]
\caption{SHAP background 10 vs 25 (median seconds per explanation, 20 test rows, CPU) and single-row \texttt{predict\_proba} latency.}
\label{tab:shap}
\centering\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}Xcccccc}
\toprule
\textbf{Disease} & \textbf{predict\_proba (ms/row)} & \textbf{bg = 10 (s)} & \textbf{bg = 25 (s)} & \textbf{Speed-up} & \textbf{Spearman (mean / min)} & \textbf{Top-3 overlap (mean / min)} \\
\midrule
Kidney (25 features) & 902 & 18.7 & 24.4 & 1.31$\times$ & 0.92 / 0.84 & 1.95 / 0 \\
Diabetes (10 features) & 1167 & 15.8 & 37.4 & 2.37$\times$ & 0.93 / 0.67 & 2.75 / 2 \\
Heart (16 features) & 702 & 18.0 & 27.0 & 1.50$\times$ & 0.96 / 0.92 & 2.70 / 2 \\
Liver (12 features) & 898 & 25.9 & 33.9 & 1.31$\times$ & 0.95 / 0.85 & 2.80 / 2 \\
\bottomrule
\end{tabularx}
\end{table*}

Cutting the background from 25 to 10 rows saved 1.3 to 2.4$\times$ on explanation time, with high mean rank agreement (Spearman 0.92--0.96). The saving and the agreement varied by dataset rather than tracking feature count simply: diabetes (10 features) gave the largest saving (2.37$\times$) but the widest per-row spread (Spearman as low as 0.67 for one patient); kidney (25 features) gave the smallest top-3 overlap (1.95 of 3) and the only case where the two background sizes agreed on \emph{none} of the top three features for one patient --- plausibly because, once many features carry near-zero attribution on this near-separable task, their relative ranking is dominated by sampling noise a larger background does not resolve; we did not test this further. KernelSHAP did not uniformly lose to the permutation explainer as the background-10 comparison in \cite{realtime2026} suggests: it beat permutation-background-25 on diabetes (16.95 s vs 37.4 s) and liver (29.63 s vs 33.9 s), and lost on heart (34.05 s vs 27.0 s) and kidney (38.1 s vs 24.4 s), with rank agreement against permutation-25 of 0.82--0.96 --- the literature finding does not transfer cleanly here; neither explainer dominates.

Table IX shows three of twelve worked examples (all twelve, with inputs, band, threshold, label, novelty flag and top contributors, are in \texttt{results/worked\_examples.md}), drawn from real held-out rows by a rule fixed in advance: the first true positive, the first true negative, and the most novel row by Mahalanobis distance. Heart's most novel row (age 59, angina absent, exercise heart rate only 90) was flagged out-of-distribution and correctly predicted disease-present at p = 0.971, driven by \texttt{ca} and \texttt{cp}. Kidney's first positive lands the exact [1.000, 1.000] band described above. Liver's most novel row (ALT 189, markedly above the rest of this patient's own panel) is flagged out-of-distribution and is a genuine error: true label disease-present, p = 0.741 sits just under the 0.752 threshold, so the served label is ``No Liver Disease''. One anecdote is not a systematic claim, but it illustrates the point of the flag: an unusual patient is exactly where a wrong answer is plausible.

\begin{table}[!t]
\caption{Selected worked examples on real held-out test rows (full set in \texttt{results/worked\_examples.md}).}
\label{tab:worked}
\centering\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccccccc}
\toprule
\textbf{Disease} & \textbf{Row} & \textbf{True} & \textbf{p} & \textbf{Band} & \textbf{Label} & \textbf{OOD} & \textbf{Top contributor} \\
\midrule
Heart & most novel & 1 & 0.971 & [0.309, 1.000] & Present & \textbf{True} & ca (+0.14) \\
Kidney & first positive & 1 & 1.000 & [1.000, 1.000] & Present & False & hemo (+0.02) \\
Liver & most novel & 1 & 0.741 & [0.083, 1.000] & \textbf{No Liver Disease (wrong)} & \textbf{True} & Sgot ($-$0.06) \\
\bottomrule
\end{tabular}}
\end{table}

\subsection{Live implementation}

The three preceding sections describe a serving path (Section III); Fig. 3 and Fig. 4 show it running locally, not mocked up, against the trained TabPFN bundle. Fig. 3 (top) is the input form for the heart-disease worked example above (Table IX, row 1), entered field-for-field into the deployed Streamlit app; Fig. 3 (bottom) is the app's own result panel for that submission --- probability 97.1\%, band 31\%-100\%, label ``Heart Disease Present'', a novelty warning (``unlike \textasciitilde{}100\% of the training cohort''), and a live SHAP bar chart topped by \texttt{ca} and \texttt{cp}. The probability, band and label match Table IX exactly, because they are the same model bundle scoring the same row; the SHAP values do not match exactly (app: ca +0.134, cp +0.099, thalach +0.050; Table IX's harness run: ca +0.139, cp +0.086, thalach +0.055) because the interactive path does not fix a random seed before sampling the k-means background (unlike the timing harness in Section V-F, which seeds every call for reproducibility) --- a gap in the shipped code this comparison surfaced. Both runs agree on the two dominant features and their sign.

Fig. 4 is the app's own ``Model Performance'' page for heart disease, reading directly from the same fixed-split artifact behind Table II: accuracy 88.5\%, F1 88.1\% and AUC 0.959 at the default 0.5 cutoff, with a second line for the learned threshold (t = 0.56: accuracy 88.5\%, precision 86.2\%, recall 89.3\%, F1 87.7\%) --- every one of these numbers reproduces a cell already in Table II or \texttt{results/tables.md}. The confusion matrix (28 true negatives, 5 false positives, 2 false negatives, 26 true positives out of n = 61) sums to the paper's own heart test-set size and accuracy exactly.

\begin{figure}[!t]\centering\includegraphics[width=\columnwidth]{figures/fig3_ui_prediction.png}\caption{Live app: the heart-disease input form filled with the Table IX ``most novel'' patient (top), and the app's own result panel for that submission --- prediction, confidence, risk level, novelty warning, gauge and SHAP chart (bottom). Captured 2026-09-23 from the Streamlit app running locally against the trained TabPFN bundle.}\label{fig:ui-prediction}\end{figure}

\begin{figure}[!t]\centering\includegraphics[width=\columnwidth]{figures/fig4_ui_performance.png}\caption{Live app: the ``Model Performance'' page for heart disease, reading the same fixed-split evaluation behind Table II (accuracy, precision, recall, F1, AUC at both the default and learned thresholds; confusion matrix; ROC and precision-recall curves).}\label{fig:ui-performance}\end{figure}
...
\begin{table*}[!t]
\caption{Verdict per design choice.}
\label{tab:verdict}
\centering\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
\toprule
\textbf{Choice} & \textbf{Compared with} & \textbf{Evidence} & \textbf{Verdict} \\
\midrule
...
Fit on train only & fit on train + test & $\Delta$AUC $\le$ 0.0015, all models & Free; chosen for correctness \\
...
Permutation SHAP, background 10 & KernelSHAP, background 25 & 1.3--2.4$\times$ faster than bg=25, Spearman 0.92--0.96; KernelSHAP faster on 2/4 tasks, slower on 2/4 & The one choice clearly paying off; KernelSHAP is not uniformly worse \\
\bottomrule
\end{tabularx}
\end{table*}
